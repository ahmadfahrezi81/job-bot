...
async def tailor_cover_letter(
    ...
    prompt = f"""Write a 150-175 word cover letter for {role_context}.

# Task
Generate body paragraphs only (no header/signature). Use the candidate's resume to select the 2 most relevant projects and describe them for this role.

# Requirements
- Length: 150-175 words (strict)
- Structure: Opening (25 words) → Body (100-120 words) → Closing (25 words)
- Tone: Direct, confident, professional (no AI clichés like "excited to apply" or "passionate about")
- Include 3+ specific metrics from resume
...
# Selection Strategy
1. Score each project in resume (0-10) based on:
   - Keyword match with job description (40%)
   - Required skills demonstrated (30%)
   - Addresses candidate gaps (20%)
   - Has quantifiable metrics (10%)

2. Select top 2 projects with scores ≥7

3. Reframe projects for this role type:
   - Data Engineer: emphasize data models, pipelines, schemas
   - Backend: emphasize APIs, architecture, scalability
   - Frontend: emphasize UI/UX, performance, responsive design
   - Full-Stack: emphasize end-to-end development
            ...
            messages=[
                {
                    "role": "system",
                    "content": (
                        "You write concise, metric-driven cover letters from resume data. "
                        "Be direct and confident. Use specific numbers. Avoid AI clichés. "
