...
async def tailor_cover_letter(
    ...
    prompt = f"""Generate a SHORT, HUMAN cover letter for {role_context} using the candidate's resume.

**CONSTRAINTS**
• Length: 175-225 words (strict)
• Tone: Direct, confident—NO AI clichés (see forbidden list below)
• Structure: Hook (50w) → Body (100-125w) → Close (25w)
• Output: Body content ONLY (no LaTeX structure/salutation/signature)
...
1. **Select 2 Projects** (score 0-10 based on):
   - JD keyword match (40%)
   - Required skills demonstrated (30%)
   - Addresses evaluation gaps (20%)
   - Has quantifiable metrics (10%)
   
   Pick TOP 2 (max 3 if generalist role). Must have different focus areas.

2. **Reframe for Role Type**:
   - Data Engineer → emphasize: pipelines, schemas, ETL, data models
   - Backend → emphasize: APIs, architecture, scalability, systems
   - QA/Automation → emphasize: testing, CI/CD, quality metrics
   - Full-Stack → emphasize: end-to-end, UI/UX, integrations
   
   Keep ALL metrics (60% reduction, 500+ users, etc.) but change angle.

3. **Extract Company Hooks** from JD:
   - Tech stack mentions
   - Product/scale details
   - Team structure
   - Recent initiatives
   
   Use ONLY if explicitly in JD. Never invent.

4. **Write 3 Paragraphs**:

   Para 1 (50w): "I__APOS__m applying for {job_title or '[Job Title]'} at {company_name or '[Company]'}. [One standout qualification with metric]"
   
   Para 2 (100-125w): Describe 2 selected projects. Each gets 1-2 sentences with 2-3 metrics. Reframe through role requirements lens.
   
   Para 3 (25w): 
   - With hooks: "I__APOS__m drawn to [Company]__APOS__s [specific JD detail]—experience I__APOS__ve gained [connection]."
   - Without hooks: "I__APOS__d welcome discussing how I can contribute to [company/goal]. Let__APOS__s connect."

**FORBIDDEN AI CLICHÉS** (auto-reject):
✗ "excited/thrilled/passionate to apply"
✗ "resonate with my passion"
✗ "eager to bring my skills"
✗ "I admire [company]'s mission"
...
**TONE RULES**
✓ Active voice: "I built", "I designed" (not "was responsible for")
✓ Specific metrics: Always include numbers
✓ Direct: "I reduced costs by 60%" (not "I was able to reduce")
✓ Natural: "I'd welcome" (not "I would be thrilled")

**SPECIAL CHARACTERS** (critical):
• Apostrophes: __APOS__ (I__APOS__ve, company__APOS__s)
• Ampersands: __AMP__ (Q__AMP__A, Cloud __AMP__ Infrastructure)
• Percents: __PCT__ (60__PCT__ reduction)
• Hashes: __HASH__, Dollars: __DOLLAR__

DO NOT use backslashes before placeholders.

**OUTPUT** (JSON):
{{
    "tailored_content": "3 paragraphs, 175-225 words, body only",
    "writing_strategy": {{
        "summary": "1-sentence approach overview",
        "opening_hook": "what hook and why",
        "body_focus": "which 2 projects, how reframed",
        "gap_handling": "how gaps addressed or why not critical"
    }},
    "change_summary": {{
        "key_customizations": "main tailoring decisions (bullets)",
        "word_count": "actual count (must be 175-225)",
        "storytelling_arc": "Hook → Project 1 → Project 2 → Close"
    }},
    "project_selection": {{
        "selected_projects": ["Project 1", "Project 2"],
        "selection_rationale": "why these scored highest",
        "relevance_scores": {{
            "Project 1": "score/10 with justification",
            "Project 2": "score/10 with justification"
        }}
    }},
    "quality_check": {{
        "length_ok": true/false,
        ...
        "metrics_included": true/false,
        "human_tone": true/false,
        "company_hooks": "specific JD details used or 'none found'"
    }}
}}

**CRITICAL**: Source from resume ONLY. Never invent experiences. Must include 3+ quantified metrics."""
            ...
            messages=[
                {
                    "role": "system",
                    "content": (
                        "You are an expert cover letter writer. Generate SHORT (175-225 words), "
                        "HUMAN cover letters from resume data. Be direct and confident. Use specific metrics. "
                        "NO AI clichés like 'excited to apply' or 'passionate about'. "
                        "Use __APOS__ for apostrophes, __AMP__ __PCT__ __HASH__ __DOLLAR__ for special characters. "
                        "Never use backslashes before placeholders. "
