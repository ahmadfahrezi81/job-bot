...
# # from services.llm_resume_service_gpt5_test import tailor_resume
...
#     3. If match_score > 70: Tailor resume
#     4. If tailored: Compile PDF and upload to Supabase
#     5. Save to Notion (with resume data + PDF URL if applicable)
#     """
#     try:
...
#         # ✅ Step 3: Tailor resume if match score > 70
#         resume_data = None
#         pdf_url = None
...
#         if match_score > 70:
#             logger.info(
#                 f"🎯 Match score {match_score}% > 70%, triggering resume tailoring..."
#             )
#             try:
#                 # Tailor resume
...
#                     pdf_bytes = await compile_resume_to_pdf(
#                         resume_data["tailored_content"]
#                     )
#                     logger.info(f"✅ PDF compiled ({len(pdf_bytes)} bytes)")

#                     # ✅ Step 5: Upload to Supabase
#                     logger.info("☁️ Uploading PDF to Supabase...")
#                     company = normalized.get("company_name", "company")
#                     job_title = normalized.get("job_title", "role")

#                     upload_result = await upload_pdf_to_supabase(
#                         pdf_bytes=pdf_bytes,
#                         position=job_title,
#                         company=company,
#                     )
...
#                     logger.error(f"⚠️ PDF compilation/upload failed: {str(pdf_error)}")
#                     # Continue without PDF - don't fail the whole pipeline

#             except Exception as e:
#                 logger.error(f"⚠️ Resume tailoring failed (continuing anyway): {str(e)}")
#                 # Continue with job save even if tailoring fails
#         else:
#             logger.info(f"⏭️ Match score {match_score}% ≤ 70, skipping resume tailoring")

#         # ✅ Step 6: Save to Notion using normalized data + resume data + PDF URL
#         job_data = {
#             "url": normalized["url"],
...
#             job_data, resume_data=resume_data, pdf_url=pdf_url
#         )
...
#             response["pdf_generated"] = pdf_url is not None
